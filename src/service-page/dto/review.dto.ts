...
import { IsDecimal, IsInt, IsNotEmpty, IsOptional, IsString, Max, Min, MaxLength, IsUUID } from 'class-validator';
...
export class CreateReviewDto {
    @ApiProperty({ description: 'Unique identifier for the service being reviewed.', example: 'd6f9a8cd-98c3-4c8b-97ad-b7d465c3e0c8' })
    @IsUUID()
    @IsNotEmpty()
    serviceId: string;

    @ApiProperty({
        description: 'Rating provided by the user for the service. Value between 0 and 5.',
        example: 4.5,
        minimum: 0,
        maximum: 5,
    })
    ...
    @ApiProperty({
        description: 'ID of the user submitting the review. Automatically fetched from the authenticated user.',
    ...
    @ApiProperty({
        description: 'Optional: Additional notes or metadata related to the review.',
        required: false,
...
export class UpdateReviewDto {
    @ApiProperty({ description: 'Unique identifier for the review.', example: 'd6f9a8cd-98c3-4c8b-97ad-b7d465c3e0c8' })
    @IsUUID()
    @IsNotEmpty()
    id: string;

    @ApiProperty({ description: 'Rating provided by the user for the service.', example: 4.5 })
    @IsDecimal()
    @Min(0)
    @Max(5)
    ...
    @ApiProperty({ description: 'Detailed review provided by the user.', example: 'The service was good but could improve in response time.' })
    @IsString()
    @MaxLength(1000)
    @IsOptional()  // Optional for updates
    review?: string;
    ...
    @ApiProperty({
        description: 'ID of the service being reviewed. This cannot be updated as the review is tied to a service.',
    ...
    @ApiProperty({
        description: 'ID of the user submitting this review. Cannot be updated as it corresponds to the review creator.',
        example: 'd96c2d57-d6fa-4fe3-b69b-23993e24a44b',
    })
    @IsUUID()
    @IsNotEmpty()
